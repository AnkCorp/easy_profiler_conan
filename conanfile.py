...
class EasyProfilerConan(ConanFile):
    ...
    url = "https://github.com/ankcorp/easy_profiler_conan"
    exports_sources = ["CMakeLists.txt", "patches/*"]
    generators = "cmake"
    settings = "os", "arch", "compiler", "build_type"
    options = {"shared": [True, False], "fPIC": [True, False], "GUI": [True, False]}
    default_options = {"shared": False, "fPIC": True, "GUI": True}
    short_paths = True
    ...
    def package(self):
        ...
        if self.settings.os == "Windows":
            for dll_file in glob.glob(
                ...
                if os.path.basename(dll_file).startswith(
                    ("concrt", "msvcp", "vcruntime")
                ):
                    os.remove(dll_file)
